...
class ADDA_Trainer(BaseTrainer):
    ...
    def _save_checkpoint(self, epoch, save_best=False):
        ...
        :param save_best: if True, rename the saved checkpoint to 'model_best.pth'
        """
        arch = type(self.tgt_encoder).__name__
        state = {
            'arch': arch,
            'epoch': epoch,
            'state_dict': self.tgt_encoder.state_dict(),
            'optimizer': [self.optimizer_enc.state_dict(), self.optimizer_dis.state_dict()],
    ...
    def _resume_checkpoint(self, resume_path):
        ...
        self.tgt_encoder.load_state_dict(checkpoint['state_dict'])
        ...
        if checkpoint['config']['optimizer']['type'] != self.config['optimizer']['type']:
            ...
            self.optimizer_enc.load_state_dict(checkpoint['optimizer'][0])
            self.optimizer_dis.load_state_dict(checkpoint['optimizer'][1])
