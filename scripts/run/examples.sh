...
while getopts "dpr" flag; do
  case "$flag" in
    ...
    r)
      ...
      EXP="MNIST_LeNet"
      RUN_ID=$VERSION
      python3 mains/main.py -c "configs/$CONFIG.json" --run_id $RUN_ID
      python3 mains/test.py -c "saved/$EXP/$RUN_ID/${CONFIG##*/}.json" --resume "saved/$EXP/$RUN_ID/model/model_best.pth" --run_id $RUN_ID
